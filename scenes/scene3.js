...
let labelA, labelB, labelAlpha, labelTheta, labelE, labelAlpha2, labelTheta2, labelEx, labelEy, labelEz, labelEyz;
...
async function initRightMap() {
  ...
  labelAlpha2 = await addLatexLabel(scene,`\\alpha`,-2,4*Math.sin(alpha)-1.6*dir.z*Math.sin(alpha/2),10-4*Math.cos(alpha)+1.6*dir.z*Math.cos(alpha/2),{size:3, color:WHITE});
  labelTheta2 = await addLatexLabel(scene,`\\theta`,-2.5,4*Math.sin(alpha)+1.5*dir.y,10-4*Math.cos(alpha)+1.5*dir.z+0.1,{size:3, color:SOFT_GREEN});
  labelEx = await addLatexLabel(scene,`-\\sin(\\theta)`,-2+2*dir.x,4*Math.sin(alpha),10-4*Math.cos(alpha)-0.2,{size:3, color:SOFT_PURPLE});
  labelEy = await addLatexLabel(scene,`\\cos(\\theta)\\cos(\\alpha)`,-1.9,4*Math.sin(alpha),10-4*Math.cos(alpha)+1.5*dir.z,{size:3, color:SOFT_PURPLE});
  labelEz = await addLatexLabel(scene,`\\cos(\\theta)\\sin(\\alpha)`,-1.9,4*Math.sin(alpha)+1.5*dir.y,10-4*Math.cos(alpha),{size:3, color:SOFT_PURPLE});
  labelEyz = await addLatexLabel(scene,`\\cos(\\theta)`,-1.9,4*Math.sin(alpha)+2.5*dir.y,10-4*Math.cos(alpha)+2.5*dir.z,{size:3, color:SOFT_YELLOW});
...
async function updateRightMap(){
  ...
  labelEx.update({x:-2+2*dir.x,y:4*Math.sin(alpha),z:10-4*Math.cos(alpha)-0.2});
  labelEy.update({x:-1.9,y:4*Math.sin(alpha),z:10-4*Math.cos(alpha)+1.5*dir.z});
  labelEz.update({x:-1.9,y:4*Math.sin(alpha)+1.5*dir.y,z:10-4*Math.cos(alpha)});
  labelEyz.update({x:-1.9,y:4*Math.sin(alpha)+2.5*dir.y,z:10-4*Math.cos(alpha)+2.5*dir.z});
