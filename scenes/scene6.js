...
let windkrachtSlider, windpuntSlider, wPercentage;
...
async function initLeftMap() {
  ...
  WLeft = drawDot3D(scene2,4*Math.sin(alpha)+d*Math.sin(beta),10-4*Math.cos(alpha)-d*Math.cos(beta),0,0.1,SOFT_PURPLE);
  endpointWLeft = drawDot3D(scene2,4*Math.sin(alpha)+d*Math.sin(beta)+W/scaleF,10-4*Math.cos(alpha)-d*Math.cos(beta),0,0.01,SOFT_PURPLE);
  ...
  labelAlphaLeft = await addLatexLabel(scene2,`\\alpha`,0.1,8.8,0,{size:4, color:WHITE});
  labelBetaLeft = await addLatexLabel(scene2,`\\beta`,4*Math.sin(alpha),10-4*Math.cos(alpha)-0.5,0,{size:4, color:WHITE});
  labelGLeft = await addLatexLabel(scene2,`\\vec{G}`,4*Math.sin(alpha)+Math.sin(beta)+0.1,10-4*Math.cos(alpha)-Math.cos(beta)-0.9*G/scaleF,0,{size:6, color:SOFT_BLUE});
  labelWLeft = await addLatexLabel(scene2,`\\vec{F}_W`,4*Math.sin(alpha)+d*Math.sin(beta)+W/scaleF-1,10-4*Math.cos(alpha)-d*Math.cos(beta)-1,0,{size:6, color:SOFT_BLUE});
  labelKLeft = await addLatexLabel(scene2,`2\\vec{F}_{K,yz}`,4*Math.sin(alpha)+K/scaleF*dir.y+0.4,10-4*Math.cos(alpha)+K/scaleF*dir.z,0,{size:6, color:SOFT_YELLOW});
  labelALeft = await addLatexLabel(scene2,`A-B`,4*Math.sin(alpha)+0.3,10-4*Math.cos(alpha),0,{size:4, color:SOFT_RED});
...
async function initRightMap() {
  ...
  //slider
    windkrachtSlider = addSlider(800, 0, 2000, {
    map: 'rightMap',
    offsetX: 30,
    offsetY: 30,
    step: 1,
    label: '\\text{Windkracht (N)}',
    isLatex:true
    });

    windpuntSlider = addSlider(33, 0, 100, {
    map: 'rightMap',
    offsetX: 30,
    offsetY: 60,
    step: 1,
    label: '\\text{Aangrijpingspunt windkracht (% hoogte)}',
    isLatex:true
    });
  ...
  pointW = drawDot3D(scene,0,4*Math.sin(alpha)+d*Math.sin(beta),10-4*Math.cos(alpha)-d*Math.cos(beta),0.1,SOFT_PURPLE);
  ...
  endpointW = drawDot3D(scene,0,4*Math.sin(alpha)+d*Math.sin(beta)+W/scaleF,10-4*Math.cos(alpha)-d*Math.cos(beta),0.01,SOFT_PURPLE);
  ...
  labelW = await addLatexLabel(scene,`\\vec{F}_{W}`,0.2,4*Math.sin(alpha)+d*Math.sin(beta)+0.9*W/scaleF,10-4*Math.cos(alpha)-d*Math.cos(beta),{size:4, color:SOFT_BLUE});
...
async function updateMaps(){
  W = windkrachtSlider.getValue();
  wPercentage = windpuntSlider.getValue();
  alpha = Math.atan(W/G);
  K = 1800/(Math.sqrt(2)*Math.cos(alpha));
  d = (100 - wPercentage)*0.02;
  beta = Math.atan(W*d/G);

  pointA.update(-2, 4*Math.sin(alpha), 10-4*Math.cos(alpha));
  pointB.update( 2, 4*Math.sin(alpha), 10-4*Math.cos(alpha));
  pointD.update(-2,4*Math.sin(alpha)+2*Math.sin(beta),10-4*Math.cos(alpha)-2*Math.cos(beta));
  pointC.update(2,4*Math.sin(alpha)+2*Math.sin(beta),10-4*Math.cos(alpha)-2*Math.cos(beta));  
  pointG.update(0,4*Math.sin(alpha)+Math.sin(beta),10-4*Math.cos(alpha)-Math.cos(beta));
  pointW.update(0,4*Math.sin(alpha)+d*Math.sin(beta),10-4*Math.cos(alpha)-d*Math.cos(beta));
  ...
  endpointK.update(-2+K/scaleF*dir.x,4*Math.sin(alpha)+K/scaleF*dir.y,10-4*Math.cos(alpha)+K/scaleF*dir.z);
  endpointKyz.update(-2,4*Math.sin(alpha)+K/scaleF*dir.y,10-4*Math.cos(alpha)+K/scaleF*dir.z);
  endpointK2.update(2-K/scaleF*dir.x,4*Math.sin(alpha)+K/scaleF*dir.y,10-4*Math.cos(alpha)+K/scaleF*dir.z);
  endpointW.update(0,4*Math.sin(alpha)+d*Math.sin(beta)+W/scaleF,10-4*Math.cos(alpha)-d*Math.cos(beta));
  endpointG.update(0,4*Math.sin(alpha)+Math.sin(beta),10-4*Math.cos(alpha)-Math.cos(beta)-G/scaleF);
  ...
  lineBC.update(pointC,pointB);
  lineCD.update(pointC,pointD);
  lineAD.update(pointA,pointD);
  
  vectorK.update(pointA, endpointK);
  vectorKyz.update(pointA, endpointKyz);
  vectorK2.update(pointB,endpointK2);
  vectorG.update(pointG,endpointG);
  vectorW.update(pointW,endpointW);
  ...
  labelK.update({x:-2+1.1*K/scaleF*dir.x,y:4*Math.sin(alpha)+1.1*K/scaleF*dir.y,z:10-4*Math.cos(alpha)+1.1*K/scaleF*dir.z});
  labelK2.update({x:2-1.1*K/scaleF*dir.x,y:4*Math.sin(alpha)+1.1*K/scaleF*dir.y,z:10-4*Math.cos(alpha)+1.1*K/scaleF*dir.z});
  labelG.update({x:0.2,y:4*Math.sin(alpha)+Math.sin(beta),z:10-4*Math.cos(alpha)-Math.cos(beta)-0.9*G/scaleF});
  labelW.update({x:0.2,y:4*Math.sin(alpha)+d*Math.sin(beta)+0.9*W/scaleF,z:10-4*Math.cos(alpha)-d*Math.cos(beta)});
  labelKyz.update({x:-1.9,y:4*Math.sin(alpha)+1.1*K/scaleF*dir.y,z:10-4*Math.cos(alpha)+1.1*K/scaleF*dir.z});


  //update left map:
  ALeft.update(4*Math.sin(alpha),10-4*Math.cos(alpha),0);
  CLeft.update(4*Math.sin(alpha)+2*Math.sin(beta),10-4*Math.cos(alpha)-2*Math.cos(beta),0);
  pointLeft2.update(4*Math.sin(alpha),10-4*Math.cos(alpha)-2*Math.cos(beta),0);
  GLeft.update(4*Math.sin(alpha)+Math.sin(beta),10-4*Math.cos(alpha)-Math.cos(beta),0);
  WLeft.update(4*Math.sin(alpha)+d*Math.sin(beta),10-4*Math.cos(alpha)-d*Math.cos(beta),0);
  endpointWLeft.update(4*Math.sin(alpha)+d*Math.sin(beta)+W/scaleF,10-4*Math.cos(alpha)-d*Math.cos(beta),0);
  endpointGLeft.update(4*Math.sin(alpha)+Math.sin(beta),10-4*Math.cos(alpha)-Math.cos(beta)-G/scaleF,0);
  endpointKyzLeft.update(4*Math.sin(alpha)+K/scaleF*dir.y,10-4*Math.cos(alpha)+K/scaleF*dir.z,0);  
  lineLeft2.update(ALeft, ULeft);  
  lineLeft3 .update(ALeft, pointLeft2);
  lineLeftAC.update(ALeft, CLeft);
  vectorKyzLeft.update(ALeft,endpointKyzLeft);
  vectorGLeft.update(GLeft,endpointGLeft);
  vectorWLeft.update(WLeft,endpointWLeft);
  let sizeAlpha2 = 4;
  if (alpha < 0.1){sizeAlpha2 = 0};
  labelAlphaLeft.update({x:0.1,y:8.8,z:0, size:sizeAlpha2});
  let sizeBeta = 4;
  if (beta < 0.1){sizeBeta = 0};
  labelBetaLeft.update({x:4*Math.sin(alpha),y:10-4*Math.cos(alpha)-0.5,z:0, size:sizeBeta});
  labelGLeft.update({x:4*Math.sin(alpha)+Math.sin(beta)+0.1,y:10-4*Math.cos(alpha)-Math.cos(beta)-0.9*G/scaleF,z:0});
  labelWLeft.update({x:4*Math.sin(alpha)+d*Math.sin(beta)+W/scaleF-1,y:10-4*Math.cos(alpha)-d*Math.cos(beta)-0.6,z:0});
  labelKLeft.update({x:4*Math.sin(alpha)+K/scaleF*dir.y+0.4,y:10-4*Math.cos(alpha)+K/scaleF*dir.z,z:0});
  labelALeft.update({x:4*Math.sin(alpha)+0.3,y:10-4*Math.cos(alpha),z:0});
  alphaArcLeft.delete();
  alphaArcLeft = drawArc(scene2,new THREE.Vector3(0,0,1),new THREE.Vector3(0,10),new THREE.Vector3(0,9),alpha,{tubeRadius:0.01});
  betaArcLeft.delete();
  betaArcLeft = drawArc(scene2,new THREE.Vector3(0,0,1),new THREE.Vector3(4*Math.sin(alpha),10-4*Math.cos(alpha)),new THREE.Vector3(4*Math.sin(alpha),10-4*Math.cos(alpha)-0.7),beta,{tubeRadius:0.01});
...
if (betaArcLeft) {
  ...
  windkrachtSlider.remove();
  windpuntSlider.remove();
}
