...
let alpha = Math.PI/6;
let pointA, pointB, pointLeftWall, pointRightWall, point1, point2, leftRope, rightRope, lineAB, line1, line2, alphaArc, thetaArc;
let labelA, labelB, labelAlpha, labelTheta;
...
async function initRightMap() {
  ...
  //Lines
  leftRope = drawThickLine3D(scene,pointLeftWall,pointA,{color:SOFT_GREEN, radius:0.05} );
  rightRope = drawThickLine3D(scene,pointRightWall,pointB,{color:SOFT_GREEN, radius:0.05} );
...
function setup3DMap(){
  ...
  controls.target.set(-2, 0, 6); // THIS is the real “look at” point for OrbitControls
  controls.update();
